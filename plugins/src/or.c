...
OMEGA_TRANSFORM_PLUGIN_EXPORT int omega_transform_plugin_apply(const omega_transform_plugin_request_t *request_ptr,
    ...
    if (omega_bitmask_parse_options(request_ptr ? request_ptr->options_json : NULL, 0x00, &mask) != 0) { return -1; }
    return omega_bitmask_apply_replace(request_ptr, response_ptr, &mask, OMEGA_BITMASK_OR);
}
