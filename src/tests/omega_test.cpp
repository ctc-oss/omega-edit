...
TEST_CASE("Empty File Test", "[EmptyFileTests]") {
    ...
    auto file_size = omega_session_get_length(session_ptr);
    REQUIRE(omega_edit_get_computed_file_size(session_ptr) == file_size);
...
TEST_CASE("Model Test", "[ModelTests]") {
    ...
    REQUIRE(omega_edit_get_num_undone_changes(session_ptr) == 1);
    auto last_undone_change = omega_edit_get_last_undo(session_ptr);
    ...
    REQUIRE(omega_edit_get_num_undone_changes(session_ptr) == 0);
    ...
    auto last_change = omega_edit_get_last_change(session_ptr);
    ...
    REQUIRE(file_info.num_changes == omega_edit_get_num_changes(session_ptr));
...
TEST_CASE("Hanoi insert", "[ModelTests]") {
    ...
    REQUIRE(0 < omega_edit_undo_last_change(session_ptr));
    REQUIRE(1 == omega_edit_get_num_undone_changes(session_ptr));
    REQUIRE(0 < omega_edit_redo_last_undo(session_ptr));
    REQUIRE(0 == omega_edit_get_num_undone_changes(session_ptr));
...
TEST_CASE("Check initialization", "[InitTests]") {
    ...
    SECTION("Open data file") {
        SECTION("Create Session") {
            ...
            REQUIRE(omega_edit_get_computed_file_size(session_ptr) == 67);
            ...
            REQUIRE(omega_edit_get_computed_file_size(session_ptr) == 70);
            ...
            REQUIRE(omega_edit_get_computed_file_size(session_ptr) == 71);
            REQUIRE(0 < omega_edit_delete(session_ptr, 9, 5));
            REQUIRE(omega_edit_get_computed_file_size(session_ptr) == 66);
            auto num_changes_before_undo = omega_edit_get_num_changes(session_ptr);
            REQUIRE(0 < omega_edit_undo_last_change(session_ptr));
            REQUIRE(omega_edit_get_num_undone_changes(session_ptr) == 1);
            REQUIRE(omega_edit_get_num_changes(session_ptr) == num_changes_before_undo - 1);
            REQUIRE(omega_edit_get_computed_file_size(session_ptr) == 71);
...
void vpt_change_cbk(const omega_viewport_t *viewport_ptr, const omega_change_t *change_ptr) {
    ...
    clog << dec << "capacity: " << omega_viewport_get_capacity(viewport_ptr)
         ...
         << " offset: " << omega_viewport_get_computed_offset(viewport_ptr)
         << " bit offset: " << static_cast<int>(omega_viewport_get_bit_offset(viewport_ptr)) << endl;
...
TEST_CASE("Search", "[SearchTests]") {
    ...
    omega_viewport_create(session_ptr, 0, 1024, vpt_change_cbk, &view_mode);
    auto needle = "needle";
    ...
    REQUIRE(omega_edit_get_num_undone_changes(session_ptr) == 1);
    REQUIRE(0 < omega_edit_undo_last_change(session_ptr));
    REQUIRE(omega_edit_get_num_undone_changes(session_ptr) == 2);
...
TEST_CASE("File Viewing", "[InitTests]") {
    ...
    viewport_ptr = omega_viewport_create(session_ptr, 0, 10, vpt_change_cbk, &view_mode, 0);
    REQUIRE(viewport_count + 1 == omega_session_get_num_viewports(session_ptr));
    view_mode.display_mode = CHAR_MODE;
    vpt_change_cbk(viewport_ptr, nullptr);
    for (int64_t offset(0); offset < omega_edit_get_computed_file_size(session_ptr); ++offset) {
    ...
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 1));
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 2));
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 3));
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 4));
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 5));
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 6));
    REQUIRE(0 == omega_viewport_update(viewport_ptr, 0, 20, 7));
    ...
    auto *buffer = (omega_byte_t *) malloc(omega_viewport_get_capacity(viewport_ptr));
    memcpy(buffer, omega_viewport_get_data(viewport_ptr), omega_viewport_get_length(viewport_ptr));
    REQUIRE(0 == omega_util_left_shift_buffer(buffer, omega_viewport_get_length(viewport_ptr), 1));
    REQUIRE(memcmp(buffer, fill + 1, omega_viewport_get_length(viewport_ptr) - 1) == 0);
    free(buffer);
    ...
    REQUIRE(0 == omega_viewport_destroy(viewport_ptr));
    REQUIRE(viewport_count - 1 == omega_session_get_num_viewports(session_ptr));
    omega_session_destroy(session_ptr);
    remove(file_name);
}
