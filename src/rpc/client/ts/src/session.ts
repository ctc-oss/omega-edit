 ...
 * scratch
 * @param desired_session_id if defined, the session ID to assign to this session, if undefined a unique session ID will
 * be generated by the server
 * @return session ID, on success
 */
export function createSession(
  file_path: string | undefined,
  desired_session_id: string | undefined
): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    let request = new CreateSessionRequest()
    if (desired_session_id && desired_session_id.length)
      request.setSessionIdDesired(desired_session_id)
    if (file_path && file_path.length) request.setFilePath(file_path)
