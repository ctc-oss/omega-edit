...
static void initialize_model_segments_(omega_model_segments_t &model_segments, int64_t length) {
    ...
    if (0 < length) {
        ...
        change_ptr->serial_ = 0;
        change_ptr->kind_ = change_kind_t::CHANGE_INSERT;
        change_ptr->offset_ = 0;
        change_ptr->length_ = length;
        auto read_segment_ptr = std::make_unique<omega_model_segment_t>();
        read_segment_ptr->change_ptr_ = change_ptr;
        read_segment_ptr->computed_offset_ = 0;
        read_segment_ptr->change_offset_ = read_segment_ptr->change_ptr_->offset_;
        read_segment_ptr->computed_length_ = read_segment_ptr->change_ptr_->length_;
...
static inline const_omega_change_ptr_t del_(int64_t serial, int64_t offset, int64_t length) {
    auto change_ptr = std::make_shared<omega_change_t>();
    change_ptr->serial_ = serial;
    change_ptr->kind_ = change_kind_t::CHANGE_DELETE;
    change_ptr->offset_ = offset;
    change_ptr->length_ = length;
    change_ptr->data_.bytes_ptr_ = nullptr;
    return change_ptr;
}
...
static inline const_omega_change_ptr_t ins_(int64_t serial, int64_t offset, const omega_byte_t *bytes, int64_t length) {
    ...
    change_ptr->kind_ = change_kind_t::CHANGE_INSERT;
    change_ptr->offset_ = offset;
    change_ptr->length_ = length ? length : static_cast<int64_t>(strlen((const char *) bytes));
    if (change_ptr->length_ < 8) {
        // small bytes optimization
        memcpy(change_ptr->data_.sm_bytes_, bytes, change_ptr->length_);
        change_ptr->data_.sm_bytes_[change_ptr->length_] = '\0';
    } else {
        // allocate its capacity plus one, so we can null-terminate it
        change_ptr->data_.bytes_ptr_ = new omega_byte_t[change_ptr->length_ + 1];
        memcpy(change_ptr->data_.bytes_ptr_, bytes, change_ptr->length_);
        change_ptr->data_.bytes_ptr_[change_ptr->length_] = '\0';
    }
    return change_ptr;
}
...
static inline const_omega_change_ptr_t ovr_(int64_t serial, int64_t offset, const omega_byte_t *bytes, int64_t length) {
    auto change_ptr = std::make_shared<omega_change_t>();
    change_ptr->serial_ = serial;
    change_ptr->kind_ = change_kind_t::CHANGE_OVERWRITE;
    change_ptr->offset_ = offset;
    change_ptr->length_ = length ? length : static_cast<int64_t>(strlen((const char *) bytes));
    if (change_ptr->length_ < 8) {
        // small bytes optimization
...
static inline void update_viewport_offset_adjustment_(omega_viewport_t *viewport_ptr,
    ...
    assert(0 < change_ptr->length_);
    // If the viewport is floating and a change happens before or at the start of the given viewport...
    if (viewport_is_floating_(viewport_ptr) && change_ptr->offset_ <= viewport_get_offset_(viewport_ptr)) {
        // ...and the change is a delete, or insert, update the offset adjustment accordingly
        if (change_kind_t::CHANGE_DELETE == change_ptr->kind_) {
            viewport_ptr->data_segment.offset_adjustment_ -= change_ptr->length_;
        } else if (change_kind_t::CHANGE_INSERT == change_ptr->kind_) {
            viewport_ptr->data_segment.offset_adjustment_ += change_ptr->length_;
        }
    }
}
...
static inline bool change_affects_viewport_(const omega_viewport_t *viewport_ptr, const omega_change_t *change_ptr) {
    ...
    switch (change_ptr->kind_) {
        ...
        case change_kind_t::CHANGE_INSERT:
            ...
            return (change_ptr->offset_ <= (viewport_get_offset_(viewport_ptr) + viewport_get_capacity_(viewport_ptr)));
        case change_kind_t::CHANGE_OVERWRITE:
            // OVERWRITE changes that happen inside the viewport affect the viewport
            return ((change_ptr->offset_ + change_ptr->length_) >= viewport_get_offset_(viewport_ptr)) &&
                   (change_ptr->offset_ <= (viewport_get_offset_(viewport_ptr) + viewport_get_capacity_(viewport_ptr)));
        default:
...
static int update_viewports_(const omega_session_t *session_ptr, const omega_change_t *change_ptr) {
    for (auto &&viewport_ptr : session_ptr->viewports_) {
        ...
        if (change_affects_viewport_(viewport_ptr.get(), change_ptr)) {
            viewport_ptr->data_segment.capacity_ =
                    -1 * std::abs(viewport_ptr->data_segment.capacity_);// indicate dirty read
            viewport_notify_(viewport_ptr.get(),
...
static inline omega_model_segment_ptr_t clone_model_segment_(const omega_model_segment_ptr_t &segment_ptr) {
    ...
    result->computed_offset_ = segment_ptr->computed_offset_;
    result->computed_length_ = segment_ptr->computed_length_;
    result->change_offset_ = segment_ptr->change_offset_;
    result->change_ptr_ = segment_ptr->change_ptr_;
    return result;
}

static inline void free_model_changes_(omega_model_struct *model_ptr) {
    for (auto &&change_ptr : model_ptr->changes_) {
        if (change_ptr->kind_ != change_kind_t::CHANGE_DELETE && 7 < change_ptr->length_) {
            delete[] const_cast<omega_change_t *>(change_ptr.get())->data_.bytes_ptr_;
        }
    }
    model_ptr->changes_.clear();
}

static inline void free_model_changes_undone_(omega_model_struct *model_ptr) {
    for (auto &&change_ptr : model_ptr->changes_undone_) {
        if (change_ptr->kind_ != change_kind_t::CHANGE_DELETE && 7 < change_ptr->length_) {
            delete[] const_cast<omega_change_t *>(change_ptr.get())->data_.bytes_ptr_;
        }
    }
    model_ptr->changes_undone_.clear();
}
...
static int clear_changes_(omega_session_t *session_ptr) {
    int64_t length = 0;
    if (session_ptr->models_.front()->file_ptr_) {
        if (0 != FSEEK(session_ptr->models_.front()->file_ptr_, 0L, SEEK_END)) { return -1; }
        length = FTELL(session_ptr->models_.front()->file_ptr_);
    }
    initialize_model_segments_(session_ptr->models_.front()->model_segments_, length);
    free_session_changes_(session_ptr);
    for (const auto &viewport_ptr : session_ptr->viewports_) {
        viewport_ptr->data_segment.capacity_ =
                -1 * std::abs(viewport_ptr->data_segment.capacity_);// indicate dirty read
        viewport_notify_(viewport_ptr.get(), VIEWPORT_EVT_CLEAR, nullptr);
    }
    session_notify_(session_ptr, SESSION_EVT_CLEAR, nullptr);
    return 0;
}

static int save_(omega_session_t *session_ptr, const char *file_path, int overwrite, char *saved_file_path) {
    ...
    for (const auto &segment : session_ptr->models_.back()->model_segments_) {
        if (write_offset != segment->computed_offset_) {
            ABORT(LOG_ERROR("break in model continuity, expected: " << write_offset
                                                                    << ", got: " << segment->computed_offset_););
        }
        switch (omega_model_segment_get_kind(segment.get())) {
            case model_segment_kind_t::SEGMENT_READ: {
                if (!session_ptr->models_.back()->file_ptr_) {
                    ABORT(LOG_ERROR("attempt to read segment from null file pointer"););
                }
                if (omega_util_write_segment_to_file(session_ptr->models_.back()->file_ptr_, segment->change_offset_,
                                                     segment->computed_length_,
                                                     temp_fptr) != segment->computed_length_) {
                    fclose(temp_fptr);
                    omega_util_remove_file(temp_filename);
            ...
            case model_segment_kind_t::SEGMENT_INSERT: {
                if (static_cast<int64_t>(
                            fwrite(omega_change_get_bytes(segment->change_ptr_.get()) + segment->change_offset_, 1,
                                   segment->computed_length_, temp_fptr)) != segment->computed_length_) {
                    fclose(temp_fptr);
                    omega_util_remove_file(temp_filename);
        ...
        write_offset += segment->computed_length_;
    }
    fclose(temp_fptr);
    ...
    if (omega_util_file_exists(file_path)) {
        if (overwrite) {
            ...
            reset_session = (get_file_path_(session_ptr) &&
                             omega_util_paths_equivalent(file_path, get_file_path_(session_ptr)));
            if (reset_session) {
                assert(session_ptr->models_.front()->file_ptr_);
                fclose(session_ptr->models_.front()->file_ptr_);
                session_ptr->models_.front()->file_ptr_ = nullptr;
            }
    ...
    if (reset_session) {
        ...
        session_ptr->models_.front()->file_ptr_ = file_ptr;
        pause_viewport_event_callbacks_(session_ptr);
        clear_changes_(session_ptr);
        resume_viewport_event_callbacks_(session_ptr);
    }
    if (saved_file_path) { omega_util_normalize_path(file_path, saved_file_path); }
    session_notify_(session_ptr, SESSION_EVT_SAVE, nullptr);
    return 0;
}

static int create_checkpoint_(omega_session_t *session_ptr, char const *checkpoint_directory) {
    ...
    const auto checkpoint_fd = omega_util_mkstemp(checkpoint_filename);
    close(checkpoint_fd);
    if (0 != save_(session_ptr, checkpoint_filename, 1, nullptr)) {
        LOG_ERROR("failed to save checkpoint file");
        return -1;
    }
    const auto file_size = get_computed_file_size_(session_ptr);
    session_ptr->num_changes_adjustment_ = get_num_changes_(session_ptr);
    session_ptr->models_.push_back(std::make_unique<omega_model_t>());
    session_ptr->models_.back()->file_ptr_ = fopen(checkpoint_filename, "rb");
    session_ptr->models_.back()->file_path_ = checkpoint_filename;
    initialize_model_segments_(session_ptr->models_.back()->model_segments_, file_size);
    session_notify_(session_ptr, SESSION_EVT_CREATE_CHECKPOINT, nullptr);
    return 0;
}
...
static int update_model_helper_(omega_model_t *model_ptr, const const_omega_change_ptr_t &change_ptr) {
    ...
    if (model_ptr->model_segments_.empty()) {
        if (change_ptr->kind_ != change_kind_t::CHANGE_DELETE) {
            // The model is empty, and we have a change with content
            auto insert_segment_ptr = std::make_unique<omega_model_segment_t>();
            insert_segment_ptr->computed_offset_ = change_ptr->offset_;
            insert_segment_ptr->computed_length_ = change_ptr->length_;
            insert_segment_ptr->change_offset_ = 0;
            insert_segment_ptr->change_ptr_ = change_ptr;
            model_ptr->model_segments_.push_back(std::move(insert_segment_ptr));
        }
        return 0;
    }
    for (auto iter = model_ptr->model_segments_.begin(); iter != model_ptr->model_segments_.end(); ++iter) {
        ...
        if (change_ptr->offset_ >= read_offset && change_ptr->offset_ <= read_offset + (*iter)->computed_length_) {
            if (change_ptr->offset_ != read_offset) {
                const auto delta = change_ptr->offset_ - (*iter)->computed_offset_;
                if (delta == (*iter)->computed_length_) {
                    ...
                    split_segment_ptr->computed_offset_ += delta;
                    split_segment_ptr->computed_length_ -= delta;
                    split_segment_ptr->change_offset_ += delta;
                    (*iter)->computed_length_ = delta;
                    // iter will now point to the new split segment inserted into the model and who's offset falls on
                    // the update site
                    iter = model_ptr->model_segments_.insert(iter + 1, std::move(split_segment_ptr));
                }
            }
            switch (change_ptr->kind_) {
                case change_kind_t::CHANGE_DELETE: {
                    auto delete_length = change_ptr->length_;
                    while (delete_length && iter != model_ptr->model_segments_.end()) {
                        if ((*iter)->computed_length_ <= delete_length) {
                            // DELETE change spans the entire segment
                            delete_length -= (*iter)->computed_length_;
                            iter = model_ptr->model_segments_.erase(iter);
                        } else {
                            // DELETE removes a portion of the beginning of the segment
                            (*iter)->computed_length_ -= delete_length;
                            (*iter)->computed_offset_ += delete_length - change_ptr->length_;
                            (*iter)->change_offset_ += delete_length;
                            assert((*iter)->change_offset_ < (*iter)->change_ptr_->length_);
                            delete_length = 0;
                            ++iter;// move to the next segment for adjusting
                        }
                    }
                    // adjust the computed offsets for segments beyond the DELETE site
                    for (; iter != model_ptr->model_segments_.end(); ++iter) {
                        (*iter)->computed_offset_ -= change_ptr->length_;
                    }
                    break;
                }
                ...
                case change_kind_t::CHANGE_INSERT: {
                    ...
                    insert_segment_ptr->change_offset_ = 0;
                    insert_segment_ptr->change_ptr_ = change_ptr;
                    iter = model_ptr->model_segments_.insert(iter, std::move(insert_segment_ptr));
                    for (++iter; iter != model_ptr->model_segments_.end(); ++iter) {
                        (*iter)->computed_offset_ += change_ptr->length_;
                    }
                    break;
                }
...
static int update_model_(omega_model_t *model_ptr, const_omega_change_ptr_t &change_ptr) {
    if (change_ptr->kind_ == change_kind_t::CHANGE_OVERWRITE) {
        // Overwrite will model just like a DELETE, followed by an INSERT
        const_omega_change_ptr_t const_change_ptr = del_(0, change_ptr->offset_, change_ptr->length_);
...
static int64_t update_(omega_session_t *session_ptr, const_omega_change_ptr_t change_ptr) {
    if (change_ptr->offset_ <= get_computed_file_size_(session_ptr)) {
        if (omega_change_get_serial(change_ptr.get()) < 0) {
            ...
            const_cast<omega_change_t *>(change_ptr.get())->serial_ *= -1;
        } else if (!session_ptr->models_.back()->changes_undone_.empty()) {
        ...
        session_ptr->models_.back()->changes_.push_back(change_ptr);
        if (0 != update_model_(session_ptr->models_.back().get(), change_ptr)) { return -1; }
        update_viewports_(session_ptr, change_ptr.get());
        session_notify_(session_ptr, SESSION_EVT_EDIT, change_ptr.get());
...
omega_session_t *omega_edit_create_session(const char *file_path, omega_session_event_cbk_t cbk, void *user_data_ptr,
    ...
    session_ptr->event_handler_ = cbk;
    session_ptr->user_data_ptr_ = user_data_ptr;
    session_ptr->event_interest_ = event_interest;
    session_ptr->num_changes_adjustment_ = 0;
    session_ptr->models_.push_back(std::make_unique<omega_model_t>());
    session_ptr->models_.back()->file_ptr_ = file_ptr;
    session_ptr->models_.back()->file_path_ =
...
void omega_edit_destroy_session(omega_session_t *session_ptr) {
    assert(session_ptr);
    {
        ...
        for (auto &&model_ptr : session_ptr->models_) {
            if (model_ptr->file_ptr_) { fclose(model_ptr->file_ptr_); }
        }
        while (!session_ptr->viewports_.empty()) { destroy_viewport_(session_ptr->viewports_.back().get()); }
        free_session_changes_(session_ptr);
        free_session_changes_undone_(session_ptr);
        while (get_num_checkpoints_(session_ptr)) {
            if (0 != omega_util_remove_file(session_ptr->models_.back()->file_path_.c_str())) {
...
omega_viewport_t *omega_edit_create_viewport(omega_session_t *session_ptr, int64_t offset, int64_t capacity,
    ...
    if (capacity > 0 && capacity <= OMEGA_VIEWPORT_CAPACITY_LIMIT) {
        ...
        viewport_ptr->data_segment.offset_ = offset;
        viewport_ptr->data_segment.offset_adjustment_ = 0;
        viewport_ptr->is_floating_ = (bool) is_floating;
        viewport_ptr->data_segment.capacity_ = -1 * capacity;// Negative capacity indicates dirty read
        viewport_ptr->data_segment.length_ = 0;
        // data segment buffer allocation is its capacity plus one, so we can null-terminate it
        viewport_ptr->data_segment.data_.bytes_ptr_ = (7 < capacity) ? new omega_byte_t[capacity + 1] : nullptr;
...
void omega_edit_destroy_viewport(omega_viewport_t *viewport_ptr) {
    assert(viewport_ptr);
    std::unique_lock ul(viewport_ptr->session_ptr->session_mutex_);
    destroy_viewport_(viewport_ptr);
}
...
int64_t omega_edit_delete(omega_session_t *session_ptr, int64_t offset, int64_t length) {
    ...
    assert(0 < length);
    std::unique_lock ul(session_ptr->session_mutex_);
    const auto computed_file_size = get_computed_file_size_(session_ptr);
    return (0 < length && offset < computed_file_size)
                   ? update_(session_ptr, del_(1 + get_num_changes_(session_ptr), offset,
...
int64_t omega_edit_insert_bytes(omega_session_t *session_ptr, int64_t offset, const omega_byte_t *bytes,
    ...
    assert(bytes);
    std::unique_lock ul(session_ptr->session_mutex_);
    return (0 <= length && offset <= get_computed_file_size_(session_ptr))
                   ? update_(session_ptr, ins_(1 + get_num_changes_(session_ptr), offset, bytes, length))
                   : 0;
}
...
int64_t omega_edit_overwrite_bytes(omega_session_t *session_ptr, int64_t offset, const omega_byte_t *bytes,
    ...
    std::unique_lock ul(session_ptr->session_mutex_);
    return (0 <= length && offset <= get_computed_file_size_(session_ptr))
                   ? update_(session_ptr, ovr_(1 + get_num_changes_(session_ptr), offset, bytes, length))
                   : 0;
}
...
int omega_edit_apply_transform(omega_session_t *session_ptr, omega_util_byte_transform_t transform, void *user_data_ptr,
    ...
    assert(0 <= length);
    std::unique_lock ul(session_ptr->session_mutex_);
    if (0 == create_checkpoint_(session_ptr, checkpoint_directory)) {
        auto in_file = session_ptr->models_.back()->file_path_;
        auto out_file = in_file + "_";
        if (0 == omega_util_apply_byte_transform_to_file(in_file.c_str(), out_file.c_str(), transform, user_data_ptr,
                                                         offset, length)) {
            errno = 0;
            if (0 == fclose(session_ptr->models_.back()->file_ptr_) && 0 == omega_util_remove_file(in_file.c_str()) &&
                0 == rename(out_file.c_str(), in_file.c_str()) &&
                (session_ptr->models_.back()->file_ptr_ = fopen(in_file.c_str(), "rb"))) {
                for (const auto &viewport_ptr : session_ptr->viewports_) {
                    viewport_ptr->data_segment.capacity_ =
                            -1 * std::abs(viewport_ptr->data_segment.capacity_);// indicate dirty read
                    viewport_notify_(viewport_ptr.get(), VIEWPORT_EVT_TRANSFORM, nullptr);
                }
                session_notify_(session_ptr, SESSION_EVT_TRANSFORM, nullptr);
                return 0;
            }
...
int omega_edit_save(omega_session_t *session_ptr, const char *file_path, int overwrite, char *saved_file_path) {
    ...
    assert(file_path);
    std::unique_lock ul(session_ptr->session_mutex_);
    return save_(session_ptr, file_path, overwrite, saved_file_path);
}

int omega_edit_clear_changes(omega_session_t *session_ptr) {
    assert(session_ptr);
    std::unique_lock ul(session_ptr->session_mutex_);
    return clear_changes_(session_ptr);
}
...
int64_t omega_edit_undo_last_change(omega_session_t *session_ptr) {
    assert(session_ptr);
    std::unique_lock ul(session_ptr->session_mutex_);
    if (!session_ptr->models_.back()->changes_.empty()) {
        const auto change_ptr = session_ptr->models_.back()->changes_.back();
        session_ptr->models_.back()->changes_.pop_back();
        int64_t length = 0;
        if (session_ptr->models_.back()->file_ptr_) {
            if (0 != FSEEK(session_ptr->models_.back()->file_ptr_, 0L, SEEK_END)) { return -1; }
            length = FTELL(session_ptr->models_.back()->file_ptr_);
        }
        initialize_model_segments_(session_ptr->models_.back()->model_segments_, length);
        for (auto iter = session_ptr->models_.back()->changes_.begin();
             iter != session_ptr->models_.back()->changes_.end(); ++iter) {
        ...
        undone_change_ptr->serial_ *= -1;

        session_ptr->models_.back()->changes_undone_.push_back(change_ptr);
        update_viewports_(session_ptr, undone_change_ptr);
        session_notify_(session_ptr, SESSION_EVT_UNDO, undone_change_ptr);
        return undone_change_ptr->serial_;
    }
    return 0;
}
...
int64_t omega_edit_redo_last_undo(omega_session_t *session_ptr) {
    ...
    int64_t rc = 0;
    std::unique_lock ul(session_ptr->session_mutex_);
    if (!session_ptr->models_.back()->changes_undone_.empty()) {
        rc = update_(session_ptr, session_ptr->models_.back()->changes_undone_.back());
        session_ptr->models_.back()->changes_undone_.pop_back();
    }
    return rc;
}
...
int omega_edit_create_checkpoint(omega_session_t *session_ptr, char const *checkpoint_directory) {
    ...
    std::unique_lock ul(session_ptr->session_mutex_);
    return create_checkpoint_(session_ptr, checkpoint_directory);
}

int omega_edit_destroy_last_checkpoint(omega_session_t *session_ptr) {
    assert(session_ptr);
    std::unique_lock ul(session_ptr->session_mutex_);
    if (get_num_checkpoints_(session_ptr)) {
        const auto last_checkpoint_ptr = session_ptr->models_.back().get();
        fclose(last_checkpoint_ptr->file_ptr_);
        if (0 != omega_util_remove_file(last_checkpoint_ptr->file_path_.c_str())) { LOG_ERROR(strerror(errno)); }
        free_model_changes_(last_checkpoint_ptr);
        free_model_changes_undone_(last_checkpoint_ptr);
        session_ptr->num_changes_adjustment_ -= (int64_t) session_ptr->models_.back()->changes_.size();
        session_ptr->models_.pop_back();
        session_notify_(session_ptr, SESSION_EVT_DESTROY_CHECKPOINT, nullptr);
        return 0;
    }
    return -1;
}
