...
omega_viewport_event_cbk_t omega_viewport_get_event_cbk_unlocked(const omega_viewport_t *viewport_ptr) {
    assert(viewport_ptr);
...
omega_viewport_event_cbk_t omega_viewport_get_event_cbk(const omega_viewport_t *viewport_ptr) {
    assert(viewport_ptr);
    std::shared_lock sl(const_cast<omega_viewport_t *>(viewport_ptr)->viewport_mutex_);
    return omega_viewport_get_event_cbk_unlocked(viewport_ptr);
}

int32_t omega_viewport_get_event_interest_unlocked(const omega_viewport_t *viewport_ptr) {
    assert(viewport_ptr);
...
int32_t omega_viewport_get_event_interest(const omega_viewport_t *viewport_ptr) {
    assert(viewport_ptr);
    std::shared_lock sl(const_cast<omega_viewport_t *>(viewport_ptr)->viewport_mutex_);
    return omega_viewport_get_event_interest_unlocked(viewport_ptr);
}

int32_t omega_viewport_set_event_interest(omega_viewport_t *viewport_ptr, int32_t event_interest) {
    assert(viewport_ptr);
    std::unique_lock ul(const_cast<omega_viewport_t *>(viewport_ptr)->viewport_mutex_);
...
int omega_viewport_update(omega_viewport_t *viewport_ptr, int64_t offset, int64_t capacity, int is_floating) {
    ...
    if (capacity > 0 && capacity <= OMEGA_VIEWPORT_CAPACITY_LIMIT) {
        ...
        if (auto const old_capacity = omega_viewport_get_capacity_unlocked(viewport_ptr);
            viewport_ptr->data_segment.offset_ != offset || old_capacity != capacity ||
            viewport_ptr->is_floating_ != (bool) is_floating) {
            if (7 < old_capacity) { delete[] viewport_ptr->data_segment.data_.bytes_ptr_; }
...
const omega_byte_t *omega_viewport_get_data_unlocked(const omega_viewport_t *viewport_ptr) {
    ...
    if (viewport_has_changes_(viewport_ptr)) {
        ...
        mut_viewport_ptr->data_segment.capacity_ = std::abs(viewport_ptr->data_segment.capacity_);
        if (populate_data_segment_(viewport_ptr->session_ptr, &mut_viewport_ptr->data_segment) != 0) { return nullptr; }
        assert(viewport_get_length_(viewport_ptr) == viewport_ptr->data_segment.length_);
    }
