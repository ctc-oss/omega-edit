...
int main(int argc, char **argv) {
    ...
    if (last_byte_info.has_last_byte) {
        for (auto i = 0; i < rotations; ++i) {
            ...
            if (ovr(author_ptr, 0, &last_byte, 1) != 0) {
                clog << "Error overwriting\n";
                return -1;
            }
            if (del(author_ptr, get_computed_file_size(session_ptr) - 1, 1) != 0) {
                clog << "Error deleting\n";
                return -1;
            }
            update_viewport(viewport_ptr, get_computed_file_size(session_ptr) - 1, 4);
        }
    }
