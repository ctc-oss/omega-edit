...
This compiles and packages the *client* and *server* directories, then creates yarn links to the package workspaces. The client build output lives under `packages/client/dist`, and the server prepackage output lives under `packages/server/out`.
...
> **NOTE**: `build.sh -d` will fully destroy any links to the *@omega-edit/client* and *@omega-edit/server* packages. This includes unlinking through `yarn unlink` and by deleting
