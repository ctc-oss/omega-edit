...
- In the 2.x line, `stopServerGraceful()` and `stopServerImmediate()` return a structured result object with `serverProcessId` and `status` instead of returning only a numeric response code.
...
The client-facing TypeScript API now uses the native-health fields listed below.
