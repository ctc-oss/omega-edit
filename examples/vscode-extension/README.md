...
- [CMake](https://cmake.org/download/), [Conan](https://conan.io), and a C/C++ compiler if you want to run the extension against locally built Ωedit™ native components
...
If you rebuild `packages/client` or `packages/server` while iterating on the extension, run `yarn vscode:setup` from the repository root again so the extension picks up the refreshed local artifacts.
...
That command installs root dependencies, builds the native core and C++ gRPC server, packages the local Node artifacts, installs this extension's npm dependencies, and overlays the freshly built local server/client tarballs without rewriting `package-lock.json`.

Useful follow-up commands from the repository root:

```bash
yarn vscode:build
yarn vscode:test:unit
yarn vscode:test:integration
yarn vscode:test
yarn vscode:package
```
...
Then open `examples/vscode-extension` in VS Code and press `F5`. A new Extension Development Host window will open.
...
VSCODE_VERSION=stable yarn vscode:test:integration
```

The underlying npm commands still work when you are already inside `examples/vscode-extension`:
