...
5. The analysis side pane profiles the current visible range or active multi-byte selection. The provider asks Ωedit™ for byte frequency, content type, language, and character-count data, while the webview derives local byte classes, top-byte summaries, entropy, and longest-run structure hints from the buffered data.
